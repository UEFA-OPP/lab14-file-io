...
# ============================================================
# Lab 14 — Save & Load Game (File I/O) Test Runner
# ============================================================
...
BUILD_DIR="$ROOT_DIR/build/saves"
...
SRC_DIR="$ROOT_DIR/assignments/saves"
TEST="$ROOT_DIR/tests/SaveManagerTest.java"
...
compile_all() {
    mkdir -p "$BUILD_DIR"
    echo -e "${DIM}[1/3] Saves файлуудыг компайл хийж байна...${RESET}"
    javac -d "$BUILD_DIR" "$SRC_DIR"/*.java 2>&1
    if [ $? -ne 0 ]; then
        echo -e "${RED}  ✗ Saves компайл алдаатай!${RESET}"
        exit 1
    fi
    echo -e "${GREEN}  ✓ Saves компайл амжилттай${RESET}"

    echo -e "${DIM}[2/3] SaveManagerTest.java компайл хийж байна...${RESET}"
...
main() {
    echo -e "${BOLD}${MAGENTA}"
    echo "  ╔═══════════════════════════════════════════════╗"
    echo "  ║  🐉 Lab 14 — Save & Load Game Grader         ║"
    echo "  ╚═══════════════════════════════════════════════╝"
    echo -e "${RESET}"
    ...
    python3 "$SCRIPT_DIR/ai_detector.py" "$SRC_DIR/SaveManager.java"
}
